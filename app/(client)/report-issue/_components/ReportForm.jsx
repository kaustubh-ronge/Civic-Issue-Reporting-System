...
// import { 
//     Loader2, MapPin, UploadCloud, ShieldAlert, CheckCircle2, 
//     X, AlertCircle, AlertTriangle, Info, Zap, Droplets, 
//     Trash2, Bus, Construction, PenTool, Copy, ExternalLink, 
//     ImageIcon, Video, Sparkles, FileEdit
// } from "lucide-react"
...
//     const handleSmartData = (aiResponse) => {
//         if (aiResponse.status === "success" && aiResponse.issues_found > 0) {
//             const issue = aiResponse.data[0]; 

//             const aiClass = issue.class_name.toLowerCase();
//             if (aiClass.includes('pothole')) setSelectedCategory("Roads");
//             else if (aiClass.includes('garbage')) setSelectedCategory("Garbage");
//             else if (aiClass.includes('pipe')) setSelectedCategory("Water");
//             else {
//                 setSelectedCategory("Other");
//                 setCustomCategoryInput(issue.class_name);
//             }

//             if (issue.severity) {
//                 setSelectedPriority(issue.severity.toUpperCase());
//             }

//             if (tags.length < 5) {
//                 setTags(prev => [...prev, `AI-${issue.severity}`]);
//             }

//             const annotatedImage = aiResponse.annotated_image || aiResponse.data?.[0]?.annotated_image || aiResponse.data?.annotated_image || null;
//             if (annotatedImage) {
//                 setAiImageData(
//                     annotatedImage.toString().startsWith('data:')
//                         ? annotatedImage.toString()
//                         : `data:image/jpeg;base64,${annotatedImage.toString()}`
//                 );
//             }

//             toast.success(`AI Auto-Filled: ${issue.class_name} (${issue.severity})`);
//         }
//     };
...
//             if (skipped.length) toast.error(`Skipped ${skipped.length} video(s) over 25MB`)
...
//                     className={`flex-1 flex items-center justify-center gap-3 p-4 rounded-xl font-bold transition-all duration-300 border-2
...
//                 <button
//                     onClick={() => setActiveMainTab("smart")}
//                     className={`flex-1 flex items-center justify-center gap-3 p-4 rounded-xl font-bold transition-all duration-300 border-2
//                         ${activeMainTab === "smart" 
//                             ? "bg-orange-600 border-orange-400 text-white shadow-[0_0_20px_rgba(249,115,22,0.3)]" 
//                             : "bg-slate-900/50 border-white/5 text-slate-400 hover:bg-slate-800 hover:text-orange-400"
//                         }`}
//                 >
//                     <Sparkles className="w-5 h-5" />
//                     Smart AI Upload Form
//                 </button>
//             </div>
...
//                         >
//                             <SmartUploadZone onAIResult={handleSmartData} />
                            
//                             {aiImageData && (
//                                 <div className="mt-4 mb-8 p-4 bg-green-500/10 border border-green-500/20 rounded-xl flex items-center gap-4">
//                                     <img src={aiImageData} alt="AI Analysis" className="w-20 h-20 object-cover rounded-lg border border-green-500/30" />
//                                     <div>
//                                         <p className="text-green-400 font-bold">AI Analysis Attached</p>
//                                         <p className="text-slate-400 text-xs">This proof will be sent directly to the administration.</p>
//                                     </div>
//                                 </div>
//                             )}
//                         </motion.div>
...
//                                 <Select onValueChange={handleCityChange} required value={selectedCity}>
...
import { 
    ...
    X, AlertCircle, AlertTriangle, Info, Zap, Droplets, 
    Trash2, Bus, Construction, PenTool, Copy, ExternalLink, 
    ImageIcon, Video, Sparkles, FileEdit, Mic
} from "lucide-react"
import { getDepartmentsByCity } from "@/actions/utilActions"
...
export default function ReportForm({ cities }) {
    ...
    const handleVoiceSubmit = async (audioBlob) => {
        ...
        toast.info("Translating and analyzing audio...")

        try {
            const formData = new FormData()
            formData.append("audio", audioBlob, "voice.webm")

            const res = await processAudioSubmission(formData)

            if (res.success) {
                setDescriptionInput(res.translated_text)
                
                // Generates a quick title from the text
                const generatedTitle = res.translated_text.split(' ').slice(0, 6).join(' ') + "..."
                setTitleInput(generatedTitle)

                if (res.category) setSelectedCategory(res.category)
                if (res.priority) setSelectedPriority(res.priority)

                toast.success("Voice processed & translated successfully!")
            } else {
                toast.error(res.error || "Failed to analyze audio.")
            }
        } catch (error) {
            toast.error("Error connecting to Voice Engine.")
        } finally {
                    ...
                    <Card className={CardStyle}>
                        <CardHeader className="pb-4 border-b border-white/5 flex flex-row items-center justify-between">
                            ...
                            <Button 
                                ...
                                className={`h-10 transition-all ${isRecording ? 'bg-red-500 hover:bg-red-600 animate-pulse' : 'bg-blue-600 hover:bg-blue-500'} text-white`}
                            >
                                {isProcessingVoice ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Mic className="h-4 w-4 mr-2" />}
                                {isProcessingVoice ? "Translating..." : isRecording ? "Listening..." : "Hold to Speak (Any Language)"}
                            </Button>
                        </CardHeader>
                        <CardContent className="pt-6 space-y-8">
                            ...
                            <div className="space-y-1">
                                ...
                                <Input 
                                    name="title" 
                                    placeholder="e.g. Broken Pipe at Station Road" 
                                    value={titleInput}
