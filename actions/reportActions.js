...
// ... KEEP YOUR EXISTING WEB FUNCTIONS HERE (uploadFileToMux, createReport, etc.) ...

// ============================================================================
// NEW: DEDICATED MOBILE SUBMISSION LOGIC
// ============================================================================
export async function createMobileReport(formData) {
    console.log("--- MOBILE SUBMISSION STARTED ---");
    try {
        const user = await checkUser();
        if (!user) return { success: false, error: "Unauthorized" };
        ...
        // Extract Mobile Video IDs (Bypass Method)
        const muxVideoIds = formData.getAll("muxVideoIds");
        console.log("Mobile Video IDs Received:", muxVideoIds);

        // Extract Tags
        const tags = formData.getAll("tags").filter(Boolean);

        // 1. Create Report & Connect Videos in one step
        const report = await db.report.create({
            data: {
                reportId,
                title: title.toString(),
                description: address ? `${description}\n\n📍 Location: ${address}` : description.toString(),
                status: "PENDING",
        ...
        // 2. Process Images to Base64 (Vercel safe limits)
        ...
        revalidatePath('/status');
        revalidatePath('/admin');
        console.log("--- MOBILE SUBMISSION SUCCESS ---");
...
// ... KEEP YOUR EXISTING HELPER FUNCTIONS HERE (getReportByReportId, getUserReports, etc.) ...
export async function getReportByReportId(reportId) {
    try {
