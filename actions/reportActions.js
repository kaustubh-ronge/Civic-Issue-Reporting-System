...
// // // // ----------------------------------------------------------------------------
// // // // NEW MOBILE SUBMISSION LOGIC (Safely appended, does not break anything)
// // // // ----------------------------------------------------------------------------
...
// // //                 department: { connect: { id: departmentId } },
// // //                 videos: {
// // //                     create: muxVideoIds.map((id, index) => ({
...
// //                         assetId = result.assetId
// //                         if (!playbackId) throw new Error("Mux returned no playback id")
// //                         storedUrl = `https://stream.mux.com/${playbackId}.mp4`
// //                     } catch (muxErr) {
// //                         console.warn('Mux upload failed, falling back to disk:', muxErr)
// //                         const outDir = path.join(process.cwd(), "public", "report-videos")
// //                         await fs.promises.mkdir(outDir, { recursive: true })
...
// // // ----------------------------------------------------------------------------
// // // DEDICATED MOBILE SUBMISSION LOGIC (Fixes the URL Null Issue)
// // // ----------------------------------------------------------------------------
// // export async function createMobileReport(formData) {
// //     try {
// //         const user = await checkUser();
...
// //         // --- THE MUX URL FIX ---
// //         // Grab the raw Upload IDs sent by the mobile app
// //         const muxVideoIds = formData.getAll("muxVideoIds");
// //         const mobileVideoData = [];

// //         // We must initialize the Mux client again here to fetch the Playback IDs
// //         const mux = new Mux({
...
// //             try {
// //                 let asset = null;
// //                 // Wait for Mux to finish processing the video the app just uploaded
// //                 for (let j = 0; j < 20; j++) {
// //                     const list = await mux.assets.list({ upload_id: uploadId, limit: 1 });
// //                     if (list.items && list.items.length > 0) {
// //                         asset = list.items[0];
...
// //                 // Map the real playback ID and generate the final .mp4 URL
// //                 if (asset && asset.status === 'ready' && asset.playback_ids?.length > 0) {
// //                     const playbackId = asset.playback_ids[0].id;
// //                     mobileVideoData.push({
// //                         playbackId: playbackId,
// //                         assetId: asset.id,
// //                         url: `https://stream.mux.com/${playbackId}.mp4`, // URL IS FILLED HERE!
// //                         order: i
// //                     });
// //                 }
// //             } catch (err) {
// //                 console.error("Mux sync error for mobile upload:", err);
// //             }
// //         }

// //         // Create Report & Connect Videos with the correct URLs
// //         const report = await db.report.create({
// //             data: {
// //                 reportId,
// //                 title,
...
// //                 department: { connect: { id: departmentId } },
// //                 videos: {
// //                     create: mobileVideoData // Now contains the fully formed URL and PlaybackId
// //                 },
// //                 ...(tags.length > 0 && {
// //                     tags: {
// //                         connectOrCreate: tags.map(tag => ({
...
// // ----------------------------------------------------------------------------
// // DEDICATED MOBILE SUBMISSION LOGIC 
// // (Fixes the Timeout Issue & Guarantees Video Row Creation)
// // ----------------------------------------------------------------------------
// export async function createMobileReport(formData) {
//     try {
//         const user = await checkUser();
...
//         // Fast loop to prepare database rows
//         for (let i = 0; i < muxVideoIds.length; i++) {
//             const uploadId = muxVideoIds[i].toString();
            
//             let actualPlaybackId = uploadId; // Fallback so DB row never fails
//             let actualAssetId = uploadId;
//             let actualUrl = null;

//             try {
//                 // VERY short poll (max 5 seconds) to prevent Vercel 10s Timeout
//                 for (let j = 0; j < 5; j++) {
//                     const list = await mux.assets.list({ upload_id: uploadId, limit: 1 });
//                     if (list.items && list.items.length > 0) {
...
//                         // Mux generates playback_ids instantly! No need to wait for 'ready'
//                         if (asset.playback_ids && asset.playback_ids.length > 0) {
//                             actualPlaybackId = asset.playback_ids[0].id;
//                             actualUrl = `https://stream.mux.com/${actualPlaybackId}.mp4`;
//                         }
//                         break; // Found it! Break the loop instantly to save time.
//                     }
//                     await new Promise(r => setTimeout(r, 1000));
//                 }
//             } catch (err) {
//                 console.error("Mux sync error for mobile:", err);
//             }

//             // GUARANTEE the row is added to the array
//             mobileVideoData.push({
//                 playbackId: actualPlaybackId,
//                 assetId: actualAssetId,
//                 url: actualUrl, // Either the correct URL or null
//                 order: i
//             });
//         }

//         // Create Report & Connect Videos
//         const report = await db.report.create({
//             data: {
//                 reportId,
//                 title,
...
//                 department: { connect: { id: departmentId } },
//                 videos: {
//                     create: mobileVideoData // Will always contain elements if videos were sent
//                 },
//                 ...(tags.length > 0 && {
//                     tags: {
//                         connectOrCreate: tags.map(tag => ({
...
export async function createMobileReport(formData) {
    try {
        ...
        // 1. UNCONDITIONAL ARRAY PUSH (Guarantees the row is always created)
        for (let i = 0; i < muxVideoIds.length; i++) {
            const uploadId = muxVideoIds[i].toString();
            
            let finalPlaybackId = uploadId; // Safety fallback
            let finalAssetId = uploadId;
            let finalUrl = null;

            try {
                // Poll Mux for up to 20 seconds, exactly like the Web version does
                for (let j = 0; j < 20; j++) {
                    const list = await mux.assets.list({ upload_id: uploadId, limit: 1 });
                    if (list.items && list.items.length > 0) {
                        const asset = list.items[0];
                        ...
                        // We do NOT wait for asset.status === 'ready'.
                        // As long as Mux gives us a playback_id, we generate the URL instantly.
                        if (asset.playback_ids && asset.playback_ids.length > 0) {
                            finalPlaybackId = asset.playback_ids[0].id;
                            finalUrl = `https://stream.mux.com/${finalPlaybackId}.mp4`;
                            break; // We got the URL, stop polling immediately to save time!
                        }
                    }
                    await new Promise(r => setTimeout(r, 1000));
                }
            ...
            // Unconditionally push to the array so the DB row is NEVER skipped
            mobileVideoData.push({
        ...
        // 2. Create Report & Connect Videos
        const report = await db.report.create({
            data: {
                reportId,
                title,
                ...
                department: { connect: { id: departmentId } },
                videos: {
                    create: mobileVideoData // This will now ALWAYS contain the video rows
                },
                ...(tags.length > 0 && {
                    tags: {
                        connectOrCreate: tags.map(tag => ({
