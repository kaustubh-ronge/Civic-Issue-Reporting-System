...
function buildDetailedAnalysis({ report, forecastSummary, generatedInsight, isBadWeather }) {
    ...
    if (report.status === "RESOLVED") {
        immediateActions.push(
            "Conduct a post-resolution quality assurance review to ensure the fix meets safety standards.",
            isBadWeather 
                ? "Closely monitor the repaired site over the next 7 days to ensure the fix withstands the upcoming severe weather." 
                : "Log the resolution details into the civic asset registry for future reference."
        );
        maintenancePlan.push(
            "Short term (0-30 days): Periodic drive-by inspections to confirm the stability of the repair.",
            "Medium term (Quarterly): Evaluate for future structural enhancements to prevent recurrence.",
            "Long term (Annual): Incorporate the site's history into city-wide preventive resilience planning."
        );
    } else {
        if (isBadWeather) {
            immediateActions.push(
        ...
        await transporter.sendMail({
            ...
            html: `
                <div style="font-family: sans-serif; padding: 20px; max-width: 600px; border: 1px solid #e2e8f0; border-radius: 8px;">
                    <div style="background-color: #0f172a; padding: 20px; border-radius: 6px; text-align: center; margin-bottom: 20px;">
                        <h2 style="color: #f97316; margin: 0;">CivicConnect Intelligence</h2>
                        <p style="color: #cbd5e1; margin: 5px 0 0 0; font-size: 14px;">Automated Environmental Audit</p>
                    </div>
                    <p style="color: #334155; font-size: 16px;">Hello,</p>
                    <p style="color: #334155; font-size: 16px; line-height: 1.5;">The AI-powered environmental risk audit for report <strong style="color: #0f172a;">${report.reportId}</strong> is ready for review.</p>
                    <div style="background-color: #f1f5f9; padding: 15px; border-left: 4px solid #3b82f6; margin: 20px 0;">
                        <p style="margin: 0 0 8px 0; color: #475569;"><strong>Issue:</strong> ${report.title}</p>
                        <p style="margin: 0; color: #475569;"><strong>Status:</strong> ${report.status}</p>
                    </div>
                    <p style="color: #334155; font-size: 16px;">The detailed PDF analysis report is attached to this email.</p>
                </div>
            `,
